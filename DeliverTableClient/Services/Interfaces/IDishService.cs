...
namespace DeliverTableClient.Services.Interfaces
{
    ...
    {
        Task<(PaginatedResult<DishDto>?, ErrorResponse?)> GetDishesByRestaurantId(int restaurantId, DishQuery query, CancellationToken cancellationToken = default);
        Task<(PaginatedResult<DishDto>?, ErrorResponse?)> GetAllDishes(DishQuery query, CancellationToken cancellationToken = default);
