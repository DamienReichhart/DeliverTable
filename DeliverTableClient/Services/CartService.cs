...
public sealed class CartService(HttpClient httpClient) : ICartService
{
    ...
    public async Task<(CartDto?, ErrorResponse?)> GetCartAsync(int restaurantId, CancellationToken ct = default)
    {
        try
        {
            var result = await _httpClient.GetFromJsonAsync<CartDto>(
    ...
    public async Task<(List<CartDto>?, ErrorResponse?)> GetAllCartsAsync(CancellationToken ct = default)
    {
        try
        {
            var result = await _httpClient.GetFromJsonAsync<List<CartDto>>(ApiRoutes.Cart.Base, ct);
            return (result, null);
        }
        catch (Exception ex)
        {
    ...
    {
        var response = await _httpClient.PostAsJsonAsync(ApiRoutes.Cart.Items, request, ct);
        ...
        var result = await response.Content.ReadFromJsonAsync<CartDto>(cancellationToken: ct);
        return (result, null);
    }
    ...
    {
        var response = await _httpClient.PutAsJsonAsync($"{ApiRoutes.Cart.Base}/items/{cartItemId}", request, ct);
        if (!response.IsSuccessStatusCode)
        {
            var error = await response.Content.ReadFromJsonAsync<ErrorResponse>(cancellationToken: ct);
            return (null, error);
        ...
        var result = await response.Content.ReadFromJsonAsync<CartItemDto>(cancellationToken: ct);
        return (result, null);
    }

    public async Task<ErrorResponse?> RemoveItemAsync(int cartItemId, CancellationToken ct = default)
    {
        var response = await _httpClient.DeleteAsync($"{ApiRoutes.Cart.Base}/items/{cartItemId}", ct);
        if (!response.IsSuccessStatusCode)
        {
    ...
    {
        var response = await _httpClient.DeleteAsync($"{ApiRoutes.Cart.Base}/restaurant/{restaurantId}", ct);
        if (!response.IsSuccessStatusCode)
        {
            return await response.Content.ReadFromJsonAsync<ErrorResponse>(cancellationToken: ct);
        }
