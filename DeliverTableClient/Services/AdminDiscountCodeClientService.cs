...
public sealed class AdminDiscountCodeClientService(HttpClient httpClient) : IAdminDiscountCodeClientService
{
    ...
    {
        using var response = await _httpClient.GetAsync(ApiRoutes.Admin.DiscountCodes, ct);
        if (!response.IsSuccessStatusCode)
            return (null, await ReadError(response, ct));

        var items = await response.Content.ReadFromJsonAsync<List<AdminDiscountCodeResponse>>(cancellationToken: ct);
    ...
    {
        using var response = await _httpClient.GetAsync($"{ApiRoutes.Admin.DiscountCodes}/{id}", ct);
        if (!response.IsSuccessStatusCode)
            return (null, await ReadError(response, ct));

        var item = await response.Content.ReadFromJsonAsync<AdminDiscountCodeResponse>(cancellationToken: ct);
        return (item, null);
    }
    ...
    {
        using var response = await _httpClient.PostAsJsonAsync(ApiRoutes.Admin.DiscountCodes, request, ct);
        if (!response.IsSuccessStatusCode)
            return (null, await ReadError(response, ct));
    ...
    {
        using var response = await _httpClient.PutAsJsonAsync($"{ApiRoutes.Admin.DiscountCodes}/{id}", request, ct);
        if (!response.IsSuccessStatusCode)
            return (null, await ReadError(response, ct));
    ...
    {
        using var response = await _httpClient.DeleteAsync($"{ApiRoutes.Admin.DiscountCodes}/{id}", ct);
        if (!response.IsSuccessStatusCode)
    ...
    public async Task<(List<AdminRedemptionResponse>? Redemptions, ErrorResponse? Error)> GetRedemptionsAsync(
        int id, CancellationToken ct = default)
    {
        using var response = await _httpClient.GetAsync($"{ApiRoutes.Admin.DiscountCodes}/{id}/redemptions", ct);
        if (!response.IsSuccessStatusCode)
            return (null, await ReadError(response, ct));

        var items = await response.Content.ReadFromJsonAsync<List<AdminRedemptionResponse>>(cancellationToken: ct);
        return items is not null
            ? (items, null)
