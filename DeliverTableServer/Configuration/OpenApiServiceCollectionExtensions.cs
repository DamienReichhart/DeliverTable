...
{
    /// <summary>
    ///     Adds OpenAPI document generation with DeliverTable-specific metadata (title, description, contact, license,
    ///     servers).
    ...
    {
        services.AddOpenApi(OpenApiConstants.DocumentName,
            options => { options.AddDocumentTransformer(new OpenApiDocumentTransformer()); });
