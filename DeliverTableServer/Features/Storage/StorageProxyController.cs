...
namespace DeliverTableServer.Features.Storage;
...
public class StorageProxyController(IObjectStorageService objectStorage) : ControllerBase
{
        ...
        string prefix, string path, CancellationToken cancellationToken)
    {
        ...
        string key = $"{prefix}/{string.Join('/', segments)}";
        ObjectStorageResult? result = await _objectStorage.GetObjectAsync(key, cancellationToken);
        ...
        string contentType = ResolveContentType(prefix, result.ContentType, out bool forceDownload);
    ...
    {
        if (AllowedContentTypes.TryGetValue(prefix, out FrozenSet<string>? allowed)
