...
public class AdminOrderController(IAdminOrderService adminOrderService, IPaymentService paymentService) : ControllerBase
{
    ...
    {
        ServiceResult<List<AdminOrderResponse>> result = await _adminOrderService.GetAllAsync(ct);
    ...
    {
        ServiceResult<AdminOrderResponse> result = await _adminOrderService.GetByIdAsync(id, ct);
        return result.ToOkResult();
    }
    ...
    {
        ServiceResult<AdminOrderResponse> result = await _adminOrderService.UpdateStatusAsync(id, request, ct);
        return result.ToOkResult();
    }
    ...
    public async Task<IActionResult> RefundOrder(int id, [FromBody] AdminRefundRequest request, CancellationToken ct)
    {
        ...
        ServiceResult<RefundDto> result = await _paymentService.RefundAsync(id, request.Amount, request.Reason, adminId, ct);
        return result.ToOkResult();
    }
}
